...
CSV_PATH = Path(
    ...
    f"{home_dir}/final_experiment/finetuned_models/finetune_pair_1/predictions.csv"
)
BASE_ID = "meta-llama/Llama-Guard-3-8B"
LORA_PATH = Path(
    f"{home_dir}/final_experiment/finetuned_models/finetune_pair_1/checkpoint-10089"
)
...
MAX_GEN_TOKENS = 200
PLATT_PATH  = f"{home_dir}/final_experiment/finetuned_models/finetune_pair_1/platt_scaler.joblib"
PI_STAR     = 0.05
PI_TRAIN    = 0.0485
...
def build_monte_carlo_prompts(
    ...
    ranked = pool.sort_values("p_star", ascending=False).head(10)
...
def load_models(use_base: bool):
    ...
    global gen_lm, clf, tok, use_base_model, platt, SHIFT_CONST
    ...
    if use_base:
        ...
        # Load Platt calibration for finetuned model
...
def classify(user_msg: str):
    ...
    if use_base_model:
        # Base model: use raw softmax probabilities
        ...
        raw_safe = logits[0, 0].item()
        print("raw_unsafe: ", raw_unsafe)
        print("raw_safe: ", raw_safe)
        p_train = platt.predict_proba([[raw_unsafe]])[0, 1]
        logit_star = math.log(p_train / (1 - p_train)) + SHIFT_CONST
...
if __name__ == "__main__":
    ...
    out_path = (
        ...
        if args.out
            ...
            f"{home_dir}/final_experiment/buffer_pair/10/"
